...
추가 문제의 날짜 배정과 재풀이는 `records.json`에 기록합니다. 다음은 형식 예시이며 실제로 해결한 뒤 추가하세요.
...
208은 신규 학습 목표, 등록 문제 수는 지정 문제 144개와 추가로 선택한 문제 수입니다. 등록 문제 해결률과 전체 목표 진도는 다릅니다. 자동 업로드된 미배정 문제는 지정/추가 문제 집계에 포함되지 않으므로 위 형식으로 날짜에 등록하세요.

## v2 계산 기준과 데이터

- 문제 식별자는 `platform:problemId`이며 CSES·프로그래머스는 번호, LeetCode는 slug입니다. `plan.md`의 84일을 `week-N-day-N`과 문제 메타데이터로 정규화해 `progress.json`의 `days`에 저장합니다. 계획 수정은 기존처럼 `plan.md`에서 하며 별도 목록을 이중 관리하지 않습니다.
- 같은 문제의 여러 파일·확장자·정답 기록은 신규 한 문제로 집계합니다. 중복 배정된 문제는 첫 날짜만 신규로 계산합니다.
- Day 완료는 지정 문제 전부 해결, 신규 목표 충족, 정답 재풀이 목표 충족이 모두 필요합니다. 신규 초과분은 재풀이 부족분을 대체하지 않습니다.
- 재풀이는 고유 `id`의 명시적 기록만 세며 `accepted`만 완료 목표에 반영합니다. `attempted`는 시도 수로 보존합니다. 실제로 다시 풀었을 때만 새 id를 만드세요. 파일 수정이나 재업로드만으로 재풀이 횟수는 늘지 않습니다.
