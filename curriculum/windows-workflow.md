...
6. 이번에 정답 확인한 문제만 묶어 메시지를 자동 생성하고 풀이와 기록 파일만 커밋합니다. 메시지 입력과 커밋 확인 질문은 없습니다.
7. 커밋 후 GitHub에 자동 push합니다. push 확인 질문도 없으며 성공하면 배치 창이 닫힙니다. 정답·힌트 사용·기존 코드 교체 여부 확인은 유지합니다.

메시지는 한 줄이며 각 구간을 실제 역슬래시(`\`)로 구분합니다. 플랫폼은 LeetCode → CSES → Programmers 순서이고 이번 업로드에 없는 플랫폼은 생략합니다.

```text
Solved Week 3 Day 5 \ LeetCode: richest-customer-wealth, running-sum-of-1d-array \ CSES: Weird Algorithm \ Programmers: Number K
```

Week·Day는 작업 세션 기준입니다. LeetCode는 URL의 slug, CSES는 영문 제목, 프로그래머스는 `curriculum/english-titles.json`의 영문 번역을 사용합니다. 현재 56개 프로그래머스 문제의 번역이 포함되어 있어 온라인 번역 서비스가 필요하지 않습니다. 새 프로그래머스 문제를 커리큘럼에 추가하면 해당 ID의 영문 제목도 이 파일에 추가하세요.

push 실패 시 로컬 커밋은 유지합니다. 연결 문제를 해결하고 UPLOAD를 다시 실행하면 새 풀이가 없어도 push를 재시도하며, 같은 기록이나 커밋을 중복 생성하지 않습니다. 원격 브랜치와 분기된 경우에는 먼저 Git의 분기를 정리해야 합니다.
